...
# Define constants
AUCTIONERS = [
    "None", "Alessandro", "Andrea", "Claudio", "Enrico", "Filippo", 
    "Giulio", "Lollo", "Luca", "Tino", "Valerio",
]
...
if page == "Auction":
    ...
    auction_results_filtered['Rank'] = auction_results_filtered['Value'].rank(method='min', ascending=False).astype(int)
    ...
    # ** Add an "Order" column based on the appearance of players for each team **
    auction_results_filtered['Order'] = auction_results_filtered.groupby('Squadra').cumcount() + 1
    ...
    for idx in new_order:
        ...
        col3.markdown(f"<div class='row-data'>{auction_results_filtered.loc[idx, 'Order']}</div>", unsafe_allow_html=True)
